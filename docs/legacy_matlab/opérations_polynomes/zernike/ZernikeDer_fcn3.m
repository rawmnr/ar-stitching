function [Z,zpfx,zpfy,n,m] = ZernikeDer_fcn3(zernv, X, Y, valid, order)
...
if exist('zernfunder.m','file') > 0
%   disp(['zernike_coeffs3.m']);
%   zernv = zernv
%   n1_M = n(zernv)
%   m1_M = m(zernv)
    [Z,zpfx,zpfy] = zernfunder(n(zernv),m(zernv),r(valid),theta(valid));  % Compute the requested Zernikes (1D)
else
