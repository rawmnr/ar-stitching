function [z,dzx,dzy] = zernfunder(n,m,r,theta,nflag)
...
%CF MODIFIED to copute derivatives
...
%put all intermediary powers (if needed)
rpowers=[max([min(rpowers)-1,0]):1:max(rpowers)];
...
dyr = zeros(length_r,length(n));
dyoverr= zeros(length_r,length(n));
for j = 1:length(n)
    s = 0:(n(j)-m_abs(j))/2;
    pows = n(j):-2:m_abs(j);
    for k = length(s):-1:1
        ...
        %now compute derivative of radial part
        if pows(k)>0
            idx2 = ((pows(k)-1)==rpowers);
            dyr(:,j) = dyr(:,j) + p*pows(k)*rpowern(:,idx2);
            dyoverr(:,j) = dyoverr(:,j) + p*rpowern(:,idx2); %1/p * R for the azimuthal part, does not work for m=0 (but we do not care)
        end
    end
    
    if isnorm
        y(:,j) = y(:,j)*sqrt((1+(m(j)~=0))*(n(j)+1)/pi); 
        dyr(:,j) = dyr(:,j)*sqrt((1+(m(j)~=0))*(n(j)+1)/pi);
    end
end
...
dzy=zeros(size(y));
if any(idx_pos)
    z(:,idx_pos) = y(:,idx_pos).*cos(theta*absm(idx_pos)');
    dyt(:,idx_pos)=- dyoverr(:,idx_pos) .*sin(theta*absm(idx_pos)').*repmat(absm(idx_pos),[1,size(theta)])';
    dzx(:,idx_pos)=dyr(:,idx_pos).*cos(theta*absm(idx_pos)').*repmat(cos(theta),[1,sum(idx_pos)]) -  dyt(:,idx_pos).*repmat(sin(theta),[1,sum(idx_pos)]);
    dzy(:,idx_pos)=dyr(:,idx_pos).*cos(theta*absm(idx_pos)').*repmat(sin(theta),[1,sum(idx_pos)]) +  dyt(:,idx_pos).*repmat(cos(theta),[1,sum(idx_pos)]);
end
if any(idx_neg)
    z(:,idx_neg) = y(:,idx_neg).*sin(theta*absm(idx_neg)');
    dyt(:,idx_neg)=dyoverr(:,idx_neg) .*cos(theta*absm(idx_neg)').*repmat(absm(idx_neg),[1,size(theta)])';
    dzx(:,idx_neg)=dyr(:,idx_neg).*sin(theta*absm(idx_neg)').*repmat(cos(theta),[1,sum(idx_neg)]) -  dyt(:,idx_neg).*repmat(sin(theta),[1,sum(idx_neg)]);
    dzy(:,idx_neg)=dyr(:,idx_neg).*sin(theta*absm(idx_neg)').*repmat(sin(theta),[1,sum(idx_neg)]) +  dyt(:,idx_neg).*repmat(cos(theta),[1,sum(idx_neg)]);
end
if any(idx_zer)
    dzx(:,idx_zer)=dyr(:,idx_zer).*repmat(cos(theta),[1,sum(idx_zer)]);
    dzy(:,idx_zer)=dyr(:,idx_zer).*repmat(sin(theta),[1,sum(idx_zer)]);
end
